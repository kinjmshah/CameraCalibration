...
def singleCamCalibration(folderName):
    ...
    maxErr = max(errorAll)
    idxMax = errorAll.index(maxErr)

    del objectPoints[idxMax]
    del imagePoints[idxMax]
    del objectPointsOriginal[idxMax]
...
def stereoCalib_2cam(folder1,folder2):
    ...
    print(len(ogP_stereo),len(imgP1_stereo),len(imgP2_stereo))
